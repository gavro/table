...
function R(d, t) {
  const e = d.getBoundingClientRect(), { width: o, height: i, x: l, y: r } = e, { clientX: h, clientY: n } = t;
  return {
    width: o,
    height: i,
    x: h - l,
  ...
  render() {
    return this.wrapper = c("div", a.CSS.popover), this.items.forEach((t, e) => {
      const o = c("div", a.CSS.item), i = c("div", a.CSS.itemIcon, {
        innerHTML: t.icon
      }), l = c("div", a.CSS.itemLabel, {
        textContent: t.label
      });
      o.dataset.index = e, o.appendChild(i), o.appendChild(l), this.wrapper.appendChild(o), this.itemEls.push(o);
...
class w {
   ...
   */
  constructor({ api: t, items: e, onOpen: o, onClose: i, cssModifier: l = "" }) {
    this.api = t, this.items = e, this.onOpen = o, this.onClose = i, this.cssModifier = l, this.popover = null, this.wrapper = this.createToolbox();
  }
  /**
   * Style classes
   */
...
class A {
  ...
  constructor(t, e, o, i) {
    this.readOnly = t, this.api = e, this.data = o, this.config = i, this.wrapper = null, this.table = null, this.toolboxColumn = this.createColumnToolbox(), this.toolboxRow = this.createRowToolbox(), this.createTableWrapper(), this.hoveredRow = 0, this.hoveredColumn = 0, this.selectedRow = 0, this.selectedColumn = 0, this.tunes = {
      ...
      const r = l.target.closest(`.${s.table}`) !== null, h = l.target.closest(`.${s.wrapper}`) === null;
      (r || h) && this.hideToolboxes();
      const u = l.target.closest(`.${s.addRow}`), p = l.target.closest(`.${s.addColumn}`);
  ...
  addColumn(t = -1, e = !1) {
    ...
    for (let r = 1; r <= this.numberOfRows; r++) {
      ...
      if (t > 0 && t <= o ? (h = this.getCell(r, t), g(n, h)) : h = this.getRow(r).appendChild(n), r === 1) {
        const u = this.getCell(r, t > 0 ? t : o + 1);
        u && e && C(u);
      }
    }
    const i = this.wrapper.querySelector(`.${s.addColumn}`);
    (l = this.config) != null && l.maxcols && this.numberOfColumns > this.config.maxcols - 1 && i && i.classList.add(s.addColumnDisabled), this.addHeadingAttrToFirstRow();
  }
  /**
  ...
  addRow(t = -1, e = !1) {
    ...
    this.fillRow(o, l), this.tunes.withHeadings && this.addHeadingAttrToFirstRow();
   ...
   */
  computeInitialSize() {
    const t = this.data && this.data.content, e = Array.isArray(t), o = e ? t.length : !1, i = e ? t.length : void 0, l = o ? t[0].length : void 0, r = Number.parseInt(this.config && this.config.rows), h = Number.parseInt(this.config && this.config.cols), n = !isNaN(r) && r > 0 ? r : void 0, u = !isNaN(h) && h > 0 ? h : void 0;
    return {
      rows: i || n || 2,
      cols: l || u || 2
    };
  ...
  updateToolboxesPosition(t = this.hoveredRow, e = this.hoveredColumn) {
    this.isColumnMenuShowing || e > 0 && e <= this.numberOfColumns && this.toolboxColumn.show(() => ({
      ...
      const o = this.getRow(t), { fromTopBorder: i } = m(this.table, o), { height: l } = o.getBoundingClientRect();
      return {
        top: `${Math.ceil(i + l / 2)}px`
      };
    });
  }
  /**
  ...
  getHoveredCell(t) {
    ...
    const { width: i, height: l, x: r, y: h } = R(this.table, t);
...
class I {
   ...
   */
  onPaste(t) {
    const e = t.detail.data, o = e.querySelector(":scope > thead, tr:first-of-type th"), l = Array.from(e.querySelectorAll("tr")).map((r) => Array.from(r.querySelectorAll("th, td")).map((n) => n.innerHTML));
    this.data = {
      withHeadings: o !== null,
      content: l
